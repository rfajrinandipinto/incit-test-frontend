...
export default function ResetPassword() {
    ...
    const validatePassword = (newPassword: string, confirmNewPassword: string) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!validation.match) {
            setError('New passwords do not match or do not meet the criteria');
            return;
        }
        ...
        try {
            ...
            if (response.ok) {
                ...
                router.push('/signin'); // Redirect to sign-in page or a success page
            } else {
                    ...
                    <main className="flex-1 bg-white">
                        <div className="py-6">
                            ...
                            <form onSubmit={handleSubmit} className="max-w-7xl mx-auto sm:px-6 lg:px-8">
                                ...
                                {error && <div style={{ color: 'red' }}>{error.split(" ").map((criteria, index) => <p key={index}>{criteria}</p>)}</div>}
                                ...
                                <button type="submit" className="inline-flex items-center px-6 py-3 border border-transparent text-base font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 mt-5">Reset Password</button>
